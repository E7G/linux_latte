...
- Read-only media queries confirmed v4l2-ctl --list-inputs lists both OV5693 (front) and T4KA3 (rear), with an AtomISP media graph. T4KA3 exposes controls and a 321468000 Hz link-frequency value. These metadata/input-enumeration results do not prove capture; the rear CSI link is disabled in the idle graph and camera streaming remains untested.
- The opt-in active-camera branch now delegates to mipad2-camera-test.sh both, whose logic skips the second sensor if the first capture fails; normal smoke runs stay non-streaming. This recovery behavior has not yet been exercised on hardware.
