...
| Intel Serial IO I2C/UART (Windows INF lists ACPI 808622C1 / 8086228A) | Defconfig enables DesignWare I2C and UART; this source snapshot omits the generic ACPI LPSS driver tables, so the Windows IDs are not confirmed as Linux ACPI matches here. | Verify controller enumeration and peripheral binding on-device; add an explicit Linux ACPI match only if the live ACPI namespace requires it. |
