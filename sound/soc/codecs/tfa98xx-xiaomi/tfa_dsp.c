...
#define pr_fmt(fmt) "tfa98xx-dsp: %s: " fmt, __func__
...
int tfaRunWriteBitfield(struct tfa98xx *tfa98xx,  struct nxpTfaBitfield bf)
{
	...
	oldvalue = (u16)snd_soc_component_read(component, bfUni.Enum.address);
	tmp = oldvalue;
	...
	pr_debug("bitfield: %s=%d (0x%x -> 0x%x)\n", tfaContBfName(bfUni.field),
		 value, tmp, oldvalue);
	snd_soc_component_write(component, bfUni.Enum.address, oldvalue);
...
int tfaRunWriteRegister(struct tfa98xx *tfa98xx, struct nxpTfaRegpatch *reg)
{
	...
	value = (u16)snd_soc_component_read(component, reg->address);
	value &= ~reg->mask;
	newvalue = reg->value & reg->mask;
	value |= newvalue;
	snd_soc_component_write(component, reg->address, value);
...
int tfa98xx_dsp_system_stable(struct tfa98xx *tfa98xx, int *ready)
{
	...
	*ready = 0;
	...
	status = (u16)snd_soc_component_read(component, TFA98XX_STATUSREG);
	sysctrl = (u16)snd_soc_component_read(component, TFA98XX_SYS_CTRL);
	...
	*ready = (status &
	 ...
	 */
	mtp0 = (u16)snd_soc_component_read(component, 0x84);
...
static int tfa98xx_clockgating(struct tfa98xx *tfa98xx, int on)
{
	...
	value = snd_soc_component_read(component, TFA98XX_CURRENTSENSE4);
	...
	return snd_soc_component_write(component, TFA98XX_CURRENTSENSE4, value);
}
...
{
	struct snd_soc_component *component = tfa98xx->component;
	u16 value, before, after, status;
	int ret;

	/* read the SystemControl register, modify the bit and write again */
	before = snd_soc_component_read(component, TFA98XX_SYS_CTRL);
	value = before;
	...
	ret = snd_soc_component_write(component, TFA98XX_SYS_CTRL, value);
	after = snd_soc_component_read(component, TFA98XX_SYS_CTRL);
	status = snd_soc_component_read(component, TFA98XX_STATUSREG);
	dev_info(&tfa98xx->i2c->dev,
		 "PWDN trace dev=%s req=%d ret=%d sys:%04x->%04x status=%04x caller=%pS\n",
		 tfa98xx->fw.name ? (char *)tfa98xx->fw.name : "?",
		 powerdown, ret, before, after, status,
			 ...
			 int num_words, int *values)
{
	...
	cf_ctrl = snd_soc_component_read(component, TFA98XX_CF_CONTROLS);
	cf_ctrl &= ~0x000E;	/* clear AIF & DMEM */
	/* set DMEM, leave AIF cleared for autoincrement */
	cf_ctrl |= (Tfa98xx_DMEM_XMEM << 1);

	snd_soc_component_write(component, TFA98XX_CF_CONTROLS, cf_ctrl);

	snd_soc_component_write(component, TFA98XX_CF_MAD, start_offset);
		       ...
		       const u8 *data)
{
	...
	return tfa98xx_bulk_write_raw(component, write_data, count);
}
				 ...
				 int value, int memtype)
{
	...
	snd_soc_component_write(component, TFA98XX_CF_CONTROLS, cf_ctrl);
	snd_soc_component_write(component, TFA98XX_CF_MAD, address & 0xffff);
...
static int tfa98xx_wait_calibration(struct tfa98xx *tfa98xx, int *done)
{
	...
	*done = 0;
	...
	mtp = snd_soc_component_read(component, TFA98XX_MTP);
...
static int tfa9887_specific(struct tfa98xx *tfa98xx)
{
	...
	value = snd_soc_component_read(component, TFA98XX_SYS_CTRL);
	/* DSP must be in control of the amplifier to avoid plops */
	value |= TFA98XX_SYS_CTRL_AMPE_MSK;
	snd_soc_component_write(component, TFA98XX_SYS_CTRL, value);

	/* some other registers must be set for optimal amplifier behaviour */
	snd_soc_component_write(component, TFA98XX_BAT_PROT, 0x13AB);
	snd_soc_component_write(component, TFA98XX_AUDIO_CTR, 0x001F);
	/* peak voltage protection is always on, but may be written */
	snd_soc_component_write(component, 0x08, 0x3C4E);
	/* TFA98XX_SYSCTRL_DCA = 0 */
	snd_soc_component_write(component, TFA98XX_SYS_CTRL, 0x024D);
	snd_soc_component_write(component, 0x0A, 0x3EC3);
	snd_soc_component_write(component, 0x41, 0x0308);
	snd_soc_component_write(component, 0x49, 0x0E82);
...
static int tfa9890_specific(struct tfa98xx *tfa98xx)
{
	...
	snd_soc_component_write(component, 0x40, 0x5a6b);
	reg = snd_soc_component_read(component, 0x59);

	reg |= 0x3;

	snd_soc_component_write(component, 0x59, reg);
	snd_soc_component_write(component, 0x40, 0x0000);
...
static int tfa9897_specific(struct tfa98xx *tfa98xx)
{
	...
	snd_soc_component_write(component, 0x48, 0x0300); /* POR value = 0x308 */

	/* $49:[0] - 1 ==> 0; CLIP - default value changed. 0 means CLIPPER on*/
	reg = snd_soc_component_read(component, TFA98XX_CURRENTSENSE4);
	reg &= ~(TFA98XX_CURRENTSENSE4_CLIP_MSK);
	snd_soc_component_write(component, TFA98XX_CURRENTSENSE4, reg);
...
static int tfa98xx_is87(struct tfa98xx *tfa98xx)
{
	...
	save_value = snd_soc_component_read(component, 0x08);
	...
	snd_soc_component_write(component, 0x08, (save_value & ~0x0400));
	check_value = snd_soc_component_read(component, 0x08);

	/* restore */
	snd_soc_component_write(component, 0x08, save_value);
...
static int tfa98xx_init(struct tfa98xx *tfa98xx)
{
	...
	snd_soc_component_write(component, TFA98XX_SYS_CTRL, TFA98XX_SYS_CTRL_I2CR_MSK);
...
int tfa98xx_restore_i2cmtp(struct tfa98xx *tfa98xx)
{
	...
	snd_soc_component_write(component, 0x40, 0x5a6b);
	readValue = (u16)snd_soc_component_read(component, 0x8b);

	readValue ^= 0x5a;

	snd_soc_component_write(component, 0x60, readValue);
	snd_soc_component_write(component, 0x0b, 0x5A);

	snd_soc_component_write(component, 0x81, 0xF800);
	snd_soc_component_write(component, 0x82, 0x414);
	snd_soc_component_write(component, 0x84, 0x3F40);
	snd_soc_component_write(component, 0x85, 0x0);
	snd_soc_component_write(component, 0x86, 0x0);
	snd_soc_component_write(component, 0x87, 0x0);
	snd_soc_component_write(component, 0x8a, 0x5555);
	snd_soc_component_write(component, 0x8b, 0xAAAA);
	snd_soc_component_write(component, 0x8c, 0x5555);
	snd_soc_component_write(component, 0x8d, 0xAAAA);

	snd_soc_component_write(component, 0x60, 0x0);
	snd_soc_component_write(component, 0x40, 0x0);
...
static int tfa98xx_startup(struct tfa98xx *tfa98xx)
{
	...
	if (tries == CFSTABLE_TRIES) {
		...
		* this indicates that the subsys is ready
		*/
		mtp0 = (u16)snd_soc_component_read(tfa98xx->component, 0x84);
...
int tfaIsCalibrated(struct tfa98xx *tfa98xx)
{
	...
	status = snd_soc_component_read(component, TFA98XX_KEY2_PROTECTED_SPKR_CAL_MTP);
...
int tfa98xx_process_patch_file(struct tfa98xx *tfa98xx, int len, const u8 *data)
{
	...
	while (index < len) {
		...
		ret = tfa98xx_bulk_write_raw(component, chunk_buf, size);
		if (ret) {
...
int float_to_int(u32 x)
{
	...
	/*
	 * Old NXP code masked the result with -(e < 32), but C still
	 * evaluates m >> e first.  Tiny/zero floats can make e >= 32 and
	 * trigger undefined behaviour.  Preserve the intended result (0)
	 * without performing an out-of-range shift.
	 */
	if (e >= 32)
		return 0;
...
int tfa98xx_set_volume(struct tfa98xx *tfa98xx, u32 voldB)
{
	...
	value = snd_soc_component_read(component, TFA98XX_AUDIO_CTR);
	...
	snd_soc_component_write(component, TFA98XX_AUDIO_CTR, value);
...
static int tfa98xx_set_mute(struct tfa98xx *tfa98xx, enum Tfa98xx_Mute mute)
{
	...
	audioctrl_value = snd_soc_component_read(component, TFA98XX_AUDIO_CTR);
	...
	ret = snd_soc_component_write(component, TFA98XX_AUDIO_CTR, audioctrl_value);
	if (ret)
		return ret;

	ret = snd_soc_component_write(component, TFA98XX_SYS_CTRL, sysctrl_value);
	return ret;
}
...
int tfa98xx_wait_result(struct tfa98xx *tfa98xx, int waitRetryCount)
{
	...
	do {
		cf_status = snd_soc_component_read(component, TFA98XX_CF_STATUS);
		tries++;
			  ...
			  u8 param_id, int len, u8 *data)
{
	 ...
	 * 1: ID, 2: parameters) */
	cf_mad = 0x0002;
	snd_soc_component_write(component, TFA98XX_CF_MAD, cf_mad);
...
static bool aec_output;
module_param(aec_output, bool, 0644);
MODULE_PARM_DESC(aec_output,
		 "enable TFA I2S/AEC data output; keep disabled on shared Mi Pad 2 bus");
...
static int tfa98xx_aec_output(struct tfa98xx *tfa98xx, int enable)
{
	...
	value = snd_soc_component_read(component, TFA98XX_I2SREG);
	if (enable == 0)
		value &= ~TFA98XX_I2SREG_I2SDOE;
	else
		value |= TFA98XX_I2SREG_I2SDOE;
	ret = snd_soc_component_write(component, TFA98XX_I2SREG, value);
...
int tfa98xx_select_mode(struct tfa98xx *tfa98xx, enum Tfa98xx_Mode mode)
{
	...
	case Tfa98xx_Mode_Normal:
		...
		ret = snd_soc_component_write(component, TFA98XX_I2SREG, i2s_value);
		if (ret == 0) {
			ret = snd_soc_component_write(component, TFA98XX_SYS_CTRL,
					    sysctrl_value);
		}
		break;
...
static int tfaRunSetCalibrateOnce(struct tfa98xx *tfa98xx)
{
	...
	if ((mtp & TFA98XX_MTP_MTPOTC) == 0) {
		...
		snd_soc_component_write(component, 0x0b, 0x5A);
		snd_soc_component_write(component, TFA98XX_MTP, 0x01);
		snd_soc_component_write(component, 0x62, 0x800);
		...
		snd_soc_component_write(component, 0x0b, 0x0);
		if (tries == TFA98XX_WAITRESULT_NTRIES) {
