...
#define pr_fmt(fmt) "tfa98xx-container: %s: " fmt, __func__
...
int tfa98xx_cnt_loadfile(struct tfa98xx *tfa98xx, int index)
{
	...
	ret = request_firmware(&container, filename, component->dev);
	if (ret) {
	...
	for (i = 0; i < data->ndev; i++) {
		...
		tfa98xx->profiles = devm_kzalloc(tfa98xx->component->dev, tfa98xx->profile_count * sizeof(struct tfaprofile), GFP_KERNEL);
		if (!tfa98xx->profiles)
			return -ENOMEM;
...
int tfaContWriteRegsProf(struct tfa98xx *tfa98xx, int profile)
{
	...
	for (i = 0; i < prof->length; i++) {
		...
		if (prof->list[i].type == dscRegister) {
			err = tfaRunWriteRegister(tfa98xx,
				(struct nxpTfaRegpatch *)(base + prof->list[i].offset));
		}
