...
static void tfa98xx_monitor(struct work_struct *work)
{
	...
	if (!defer_dsp_start && tfa98xx->desired_running) {
		if (tfa98xx->dsp_init != TFA98XX_DSP_INIT_DONE) {
			/*
			 * Follow the Xiaomi/NXP factory startup order.  The factory
			 * tfa98xx_startup() writes the container register setup first,
			 * clears PWDN, waits for AREF/PLL, then brings up CoolFlux.
			 * Waiting for PLLS/AREFS here while the chip is still in PWDN
			 * deadlocks startup.
			 */
...
static void tfa98xx_dsp_init(struct work_struct *work)
{
	...
	/* start the DSP using the latest profile / vstep */
	{
		...
		ret = tfa98xx_dsp_start(tfa98xx, tfa98xx->profile_ctl,
				      tfa98xx->vstep_ctl);
		if (!ret)
			   ...
			   struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_START:
		/*
		 * The original Xiaomi driver deferred DSP init until I2S clocks
		 * are running. Keep safe-probe mode passive, but once explicitly
		 * enabled allow normal Linux playback to kick the factory DSP.
		 */
		if (!defer_dsp_start)
			queue_work(tfa98xx->tfa98xx_wq, &tfa98xx->init_work);
		break;
