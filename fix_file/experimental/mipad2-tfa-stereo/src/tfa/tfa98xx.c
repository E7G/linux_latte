...
static bool defer_dsp_start = true;
module_param(defer_dsp_start, bool, 0644);
MODULE_PARM_DESC(defer_dsp_start,
		 "defer CoolFlux DSP start for safe probe/bring-up");

static DEFINE_MUTEX(tfa98xx_instances_lock);
static struct tfa98xx *tfa98xx_instances[2];

/* Called by the Mi Pad 2 machine driver from its RT5659 AIF2 stream ops. */
void tfa98xx_set_playback_active(bool active)
{
	unsigned int i;

	mutex_lock(&tfa98xx_instances_lock);
	for (i = 0; i < ARRAY_SIZE(tfa98xx_instances); i++) {
		struct tfa98xx *tfa98xx = tfa98xx_instances[i];

		if (!tfa98xx)
			continue;

		WRITE_ONCE(tfa98xx->playback_active, active);
		if (active) {
			WRITE_ONCE(tfa98xx->monitor_status, 1);
			mod_delayed_work(tfa98xx->tfa98xx_wq,
					 &tfa98xx->delay_work, 0);
		} else if (READ_ONCE(tfa98xx->monitor_status)) {
			/* Check shortly after STOP and park the DSP while idle. */
			mod_delayed_work(tfa98xx->tfa98xx_wq,
					 &tfa98xx->delay_work, HZ);
		}
	}
	mutex_unlock(&tfa98xx_instances_lock);
}
EXPORT_SYMBOL_GPL(tfa98xx_set_playback_active);

static int tfa98xx_instance_index(struct device *dev)
{
	const char *name = dev_name(dev);

	if (!strcmp(name, "i2c-tfa9890:00"))
		return 0;
	if (!strcmp(name, "i2c-tfa9890:01"))
		return 1;
	return -EINVAL;
}

static int tfa98xx_start_with_stereo_slot(struct tfa98xx *tfa98xx,
					 int profile, int vstep)
{
	int channel, ret;

	ret = tfa98xx_dsp_start(tfa98xx, profile, vstep);
	if (ret)
		return ret;

	/* Factory containers leave CHSA=0 on both chips: both speakers then
	 * reproduce the left I2S slot. Select slot 1 on the right amplifier
	 * after every DSP start, including profile and volume-step changes.
	 */
	channel = tfa98xx_instance_index(&tfa98xx->i2c->dev);
	if (channel < 0)
		return channel;
	ret = snd_soc_component_update_bits(tfa98xx->component,
		TFA98XX_I2SREG, TFA98XX_I2SREG_CHSA_MSK,
		channel << TFA98XX_I2SREG_CHSA_POS);
	if (ret < 0)
		return ret;
	dev_info(&tfa98xx->i2c->dev,
		 "stereo I2S slot %d selected, I2SREG=0x%04x\n",
		 channel, snd_soc_component_read(tfa98xx->component,
					     TFA98XX_I2SREG));
	return 0;
}
...
int tfa98xx_bulk_write_raw(struct snd_soc_component *component, const u8 *data,
				u8 count)
{
...
static void tfa98xx_monitor(struct work_struct *work)
{
	...
	val = snd_soc_component_read(tfa98xx->component, TFA98XX_STATUSREG);

	if (!defer_dsp_start && tfa98xx->desired_running) {
		if (!READ_ONCE(tfa98xx->playback_active)) {
			if (tfa98xx->dsp_init == TFA98XX_DSP_INIT_DONE) {
				dev_info(&tfa98xx->i2c->dev,
					 "playback idle; parking TFA DSP\n");
				if (!tfa98xx_is_pwdn(tfa98xx))
					tfa98xx_dsp_stop(tfa98xx);
				tfa98xx->dsp_init = TFA98XX_DSP_INIT_PENDING;
			}
			next_delay = HZ;
		} else if (tfa98xx->dsp_init != TFA98XX_DSP_INIT_DONE) {
			/*
			 * Preserve the proven Xiaomi/NXP startup order: factory DSP
			 * startup clears PWDN first and then waits for AREF/PLL.
			 *
			 * The missing condition in finalfix was not PLLS/AREFS but
			 * whether a playback stream is actually active.  After clocks
			 * disappear we keep the DSP pending, but do not retry startup
			 * every monitor tick while PipeWire is idle.
			 */
			queue_work(tfa98xx->tfa98xx_wq, &tfa98xx->init_work);
			next_delay = HZ;
		} else if ((val & ready) != ready) {
			/*
			 * Speaker route may stay selected while PipeWire is idle.
			 * Power down when AIF2 clocks disappear, but retain
			 * desired_running so playback can wake the amp again.
			 */
			tfa98xx_dsp_stop(tfa98xx);
			tfa98xx->dsp_init = TFA98XX_DSP_INIT_PENDING;
			next_delay = msecs_to_jiffies(100);
		} else if ((TFA98XX_STATUSREG_ACS & val) ||
			   (TFA98XX_STATUSREG_WDS & val)) {
			tfa98xx->dsp_init = TFA98XX_DSP_INIT_RECOVER;

			if (TFA98XX_STATUSREG_ACS & val)
				dev_warn(&tfa98xx->i2c->dev, "DSP recovery: ACS set\n");
			if (TFA98XX_STATUSREG_WDS & val)
				dev_warn(&tfa98xx->i2c->dev, "DSP recovery: WDS set\n");
...
static void tfa98xx_dsp_init(struct work_struct *work)
{
	...
	if (defer_dsp_start) {
		dev_info(&tfa98xx->i2c->dev,
			 "DSP start deferred (safe probe mode)\n");
		return;
	}
	...
	{
		int ret;
		u16 status, sysctrl, cfstatus;

		ret = tfa98xx_start_with_stereo_slot(tfa98xx, tfa98xx->profile_ctl,
				      tfa98xx->vstep_ctl);
		if (!ret)
		...
		status = snd_soc_component_read(tfa98xx->component, TFA98XX_STATUSREG);
		sysctrl = snd_soc_component_read(tfa98xx->component, TFA98XX_SYS_CTRL);
		cfstatus = snd_soc_component_read(tfa98xx->component, TFA98XX_CF_STATUS);
		dev_info(&tfa98xx->i2c->dev,
			 "factory DSP init ret=%d profile=%d vstep=%d status=0x%04x sys=0x%04x cf=0x%04x\n",
			 ret, tfa98xx->profile_ctl, tfa98xx->vstep_ctl,
			 status, sysctrl, cfstatus);
	}
...
static int tfa98xx_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	val = snd_soc_component_read(component, TFA98XX_AUDIOREG);
	...
	snd_soc_component_write(component, TFA98XX_AUDIOREG, val);
			     ...
			     struct snd_soc_dai *dai)
{
	...
	/* The Mi Pad 2 speaker C2C links do not reliably deliver PCM trigger
	 * callbacks. hw_params is the first reliable indication that playback
	 * has activated the TFA back-end. Keep this set until hw_free/STOP.
	 */
	WRITE_ONCE(tfa98xx->playback_active, true);
	dev_info(&tfa98xx->i2c->dev,
		 "PCM hw_params active rate=%d\n", params_rate(params));

	return 0;
}

static int tfa98xx_hw_free(struct snd_pcm_substream *substream,
			   struct snd_soc_dai *dai)
{
	struct tfa98xx *tfa98xx = snd_soc_component_get_drvdata(dai->component);

	WRITE_ONCE(tfa98xx->playback_active, false);
	dev_info(&tfa98xx->i2c->dev, "PCM hw_free active=0\n");
	return 0;
}
				   ...
				   struct snd_soc_dai *dai)
{
	...
	pr_debug("\n");
	WRITE_ONCE(tfa98xx->playback_active, false);
	dev_info(&tfa98xx->i2c->dev, "PCM shutdown active=0\n");
}
			   ...
			   struct snd_soc_dai *dai)
{
	...
	dev_info(&tfa98xx->i2c->dev, "PCM trigger cmd=%d\n", cmd);
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		WRITE_ONCE(tfa98xx->playback_active, true);
		/*
		 * Wake the monitor rather than starting the DSP unconditionally.
		 * Once Stop=off expresses desired_running, the monitor will queue
		 * factory startup only while this playback stream is active.
		 */
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	*/
	...
	if (!defer_dsp_start && tfa98xx_is_amp_running(tfa98xx)) {
		/*
		...
		tfa98xx_start_with_stereo_slot(tfa98xx, tfa98xx->profile_ctl,
				  prof_new->vstep);
	}
...
static int tfa98xx_probe(struct snd_soc_component *component)
{
	 ...
	 */
	snd_soc_component_read(component, TFA98XX_REVISIONNUMBER);

	rev = snd_soc_component_read(component, TFA98XX_REVISIONNUMBER);
	dev_dbg(component->dev, "ID revision 0x%04x\n", rev);
	...
	snd_soc_add_component_controls(component, tfa98xx_controls,
				   ARRAY_SIZE(tfa98xx_controls));

	dev_dbg(component->dev, "tfa98xx component(%s) registered",
		dev_name(&tfa98xx->i2c->dev));
...
static void tfa98xx_remove(struct snd_soc_component *component)
{
	...
	dev_info(component->dev, "tfa98xx component removed");
	WRITE_ONCE(tfa98xx->monitor_status, 0);
	WRITE_ONCE(tfa98xx->desired_running, false);
	WRITE_ONCE(tfa98xx->playback_active, false);
	cancel_delayed_work_sync(&tfa98xx->delay_work);
...
static const struct snd_soc_component_driver tfa98xx_soc_component = {
	.probe = tfa98xx_probe,
...
static DEVICE_ATTR(monitor, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH,
...
static int tfa98xx_i2c_probe(struct i2c_client *i2c)
{
	...
	*dai_drv = tfa98xx_dai;
	...
	ret = devm_snd_soc_register_component(&i2c->dev, &tfa98xx_soc_component,
			dai_drv, 1);
	if (ret < 0) {
		pr_err("%s: Error registering tfa98xx component", __func__);
		goto codec_fail;
	}
	...
	slot = tfa98xx_instance_index(&i2c->dev);
	if (slot >= 0) {
		mutex_lock(&tfa98xx_instances_lock);
		tfa98xx_instances[slot] = tfa98xx;
		mutex_unlock(&tfa98xx_instances_lock);
		dev_info(&i2c->dev, "registered Mi Pad 2 stream activity hook\n");
	} else {
		dev_warn(&i2c->dev, "no Mi Pad 2 stream activity hook for this device\n");
	}
...
static void tfa98xx_i2c_remove(struct i2c_client *client)
{
	struct tfa98xx *tfa98xx = i2c_get_clientdata(client);
	int slot = tfa98xx_instance_index(&client->dev);

	device_remove_file(&client->dev, &dev_attr_monitor);
	if (slot >= 0) {
		mutex_lock(&tfa98xx_instances_lock);
		if (tfa98xx_instances[slot] == tfa98xx)
			tfa98xx_instances[slot] = NULL;
		mutex_unlock(&tfa98xx_instances_lock);
	}

	/* Stop producers and drain timer/work callbacks before freeing the queue.
	 * destroy_workqueue() alone does not cancel delayed-work timers; a pending
	 * timer can otherwise enqueue onto the just-destroyed tfa98xx queue.
	 */
	WRITE_ONCE(tfa98xx->monitor_status, 0);
	WRITE_ONCE(tfa98xx->desired_running, false);
	WRITE_ONCE(tfa98xx->playback_active, false);
...
MODULE_DESCRIPTION("ASoC tfa98xx component driver");
MODULE_LICENSE("GPL");
MODULE_AUTHOR("NXP");
