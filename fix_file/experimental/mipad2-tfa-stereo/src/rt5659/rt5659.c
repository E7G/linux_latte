	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	case RT5659_AIF2:
		...
		dev_info(component->dev,
			 "Mi Pad 2 AIF2 stream active; woke TFA DSP monitors\n");
		break;
	case RT5659_AIF3:
...
static int rt5659_set_bclk_ratio(struct snd_soc_dai *dai, unsigned int ratio)
{
	...
	/*
	 * bclk[] stores the actual BCLK frequency in Hz.  Do not overwrite it
	 * with the dimensionless 32/64 ratio, otherwise runtime switching leaves
	 * the codec state inconsistent with hw_params().
	 */
	if (rt5659->lrck[dai->id])
		rt5659->bclk[dai->id] = rt5659->lrck[dai->id] * ratio;

	switch (dai->id) {
	case RT5659_AIF2:
		snd_soc_component_update_bits(component, RT5659_ADDA_CLK_1,
			RT5659_I2S_BCLK_MS2_MASK,
			ratio == 64 ? RT5659_I2S_BCLK_MS2_64 :
				      RT5659_I2S_BCLK_MS2_32);
		break;
	case RT5659_AIF3:
		snd_soc_component_update_bits(component, RT5659_ADDA_CLK_1,
			RT5659_I2S_BCLK_MS3_MASK,
			ratio == 64 ? RT5659_I2S_BCLK_MS3_64 :
...
static int rt5659_aif_hw_free(struct snd_pcm_substream *substream,
			      struct snd_soc_dai *dai)
{
	if (dai->id == RT5659_AIF2) {
		tfa98xx_set_playback_active(false);
		dev_info(dai->component->dev,
			 "Mi Pad 2 AIF2 stream idle; parked TFA DSP monitors\n");
	}

	return 0;
}

static int rt5659_aif_trigger(struct snd_pcm_substream *substream, int cmd,
			      struct snd_soc_dai *dai)
{
	bool active;

	dev_info(dai->component->dev,
		 "Mi Pad 2 DAI trigger id=%d cmd=%d\n", dai->id, cmd);
	if (dai->id != RT5659_AIF1 && dai->id != RT5659_AIF2)
		return 0;
	...
	dev_info(dai->component->dev,
		 "Mi Pad 2 AIF%d trigger cmd=%d active=%d\n", dai->id, cmd, active);
	tfa98xx_set_playback_active(active);
	return 0;
}

static const struct snd_soc_dai_ops rt5659_aif_dai_ops = {
	.hw_params = rt5659_hw_params,
	.hw_free = rt5659_aif_hw_free,
	.trigger = rt5659_aif_trigger,
