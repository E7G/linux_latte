...
static int mipad2_aif2_bclk_ratio = 64;

static int mipad2_set_aif2_bclk_ratio(const char *val,
				      const struct kernel_param *kp)
{
	...
	ret = kstrtoint(val, 0, &ratio);
	if (ret)
		return ret;
	if (ratio != 32 && ratio != 64)
		return -EINVAL;

	for (i = 0; i < snd_soc_card_cht.num_links; i++) {
		if (snd_soc_card_cht.dai_link[i].name &&
		    !strcmp(snd_soc_card_cht.dai_link[i].name,
			    "Codec AIF2 Port")) {
			rtd = snd_soc_get_pcm_runtime(&snd_soc_card_cht,
						      &snd_soc_card_cht.dai_link[i]);
			break;
		}
	}
	if (!rtd)
	...
	mipad2_aif2_bclk_ratio = ratio;
	dev_info(snd_soc_card_cht.dev,
		 "Mi Pad 2 RT5659 AIF2 BCLK switched live: %dfs\n",
		 ratio);
	return 0;
}

static const struct kernel_param_ops mipad2_aif2_bclk_ops = {
	.set = mipad2_set_aif2_bclk_ratio,
	.get = param_get_int,
};

module_param_cb(mipad2_aif2_bclk_ratio, &mipad2_aif2_bclk_ops,
		&mipad2_aif2_bclk_ratio, 0644);
MODULE_PARM_DESC(mipad2_aif2_bclk_ratio,
		 "Mi Pad 2 RT5659 AIF2 BCLK ratio: 32 or 64; default 64");
...
static int snd_cht_mc_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "Mi Pad 2 Android TFA AIF2 topology enabled (manual BCLK switch)\n");
